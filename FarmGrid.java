...
public class FarmGrid extends Grid<Farm>
{
    ...
    private void init()
    {
        ...
        final Column<Farm> adressColumn =  addColumn(Farm::getAddress).setResizable(true).setSortable(true).setHeader("Адрес");
        addColumn(farm -> {
        ...
        HeaderRow headerRoww = appendHeaderRow();
        final TextField adressFilterField = new TextField();
        adressFilterField.setPlaceholder("Въведете адрес за филтър");
        adressFilterField.setValueChangeMode(ValueChangeMode.EAGER);
        adressFilterField.setClearButtonVisible(true);
        adressFilterField.addThemeVariants(TextFieldVariant.LUMO_SMALL);
        adressFilterField.setWidthFull();
        adressFilterField.getStyle().set("max-width", "100%");
        adressFilterField.addValueChangeListener(
                        e -> {
                            farmListDataProvider.clearFilters();
                            farmListDataProvider.addFilter(f -> {
                                                               final String adress = f.getAddress().toLowerCase();
                                                               final String value = e.getValue();
                                                               if (StringUtils.isNotBlank(value))
                                                               {
                                                                   return adress.contains(value.toLowerCase());
                                                               }
                                                               return true;
                                                           }
                            );
        ...
        refreshGrid(farms);
        headerRow.getCell(adressColumn).setComponent(adressFilterField);
        refreshGrid(farms);
    }
